...
from tgbot.data_base.db_position import Positionx
from tgbot.data_base.db_purchases import Purchasesx, PurchasesModel
from tgbot.data_base.db_refill import Refillx, RefillModel
from tgbot.data_base.db_settings import Settingsx
from tgbot.data_base.db_users import Userx, UserModel
...
async def open_profile_user(bot: Bot, user_id: int | str):
...
async def position_open_user(bot: Bot, user_id: int, position_id: str | int, remover: str | int):
...
async def category_open_admin(bot: Bot, user_id: int, category_id: str | int, remover: int):
...
async def position_open_admin(bot: Bot, user_id: int, position_id: str | int):
    get_items = Itemx.gets(position_id=position_id)
    get_position = Positionx.get(position_id=position_id)
    get_category = Categoryx.get(category_id=get_position.category_id)
...
async def item_open_admin(bot: Bot, user_id: int, item_id: str | int):
...
def get_statistics() -> str:
    ...
    week_day = int(datetime.fromtimestamp(unix_week, pytz.timezone(settings.BOT_TIMEZONE)).strftime("%d"))
    week_month = int(datetime.fromtimestamp(unix_week, pytz.timezone(settings.BOT_TIMEZONE)).strftime("%m"))
    week_week = int(datetime.fromtimestamp(unix_week, pytz.timezone(settings.BOT_TIMEZONE)).weekday())
