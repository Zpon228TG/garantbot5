...
from ..bot_settings import settings, Logger
...
async def send_admins(bot: Bot, text: str) -> None:
    await bot.send_message(chat_id=settings.LOGS_UR.get_secret_value(), text=text)


# Получение даты
def get_date(full: bool = True) -> str:
    if full:  # Полная дата с временем
        return datetime.now(pytz.timezone(settings.BOT_TIMEZONE)).strftime("%d.%m.%Y %H:%M:%S")
    else:  # Только дата без времени
        return datetime.now(pytz.timezone(settings.BOT_TIMEZONE)).strftime("%d.%m.%Y")
...
def convert_date(from_time, full=True, second=True) -> str | int:
    ...
    if str(from_time).isdigit():
        if full:
            to_time = datetime.fromtimestamp(from_time, pytz.timezone(settings.BOT_TIMEZONE)).strftime("%d.%m.%Y %H:%M:%S")
        elif second:
            to_time = datetime.fromtimestamp(from_time, pytz.timezone(settings.BOT_TIMEZONE)).strftime("%d.%m.%Y %H:%M")
        else:
            to_time = datetime.fromtimestamp(from_time, pytz.timezone(settings.BOT_TIMEZONE)).strftime("%d.%m.%Y")
    else:
        if " " in str(from_time):
...
def to_number(get_number, remains=2) -> int | float:
