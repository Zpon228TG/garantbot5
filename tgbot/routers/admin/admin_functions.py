...
from tgbot.bot_settings import FSM, ARS
from tgbot.data_base.db_purchases import Purchasesx
from tgbot.data_base.db_refill import Refillx
...
async def functions_search(message: Message, state: FSM) -> None:
    await state.clear()
...
async def functions_mail(message: Message, state: FSM) -> None:
    await state.clear()
...
async def functions_search_get(message: Message, bot: Bot, state: FSM, ar_session: ARS) -> Message | None:
    ...
    if get_purchase is not None:
        return await purchase_open_admin(bot, ar_session, message.from_user.id, get_purchase)
