...
class CrystalPayAPI:
    ...
    async def generate_pay_link(self, amount: float) -> str:
        ...
        async with aiohttp.ClientSession(headers=self.headers, timeout=self.timeout) as session:
            ...
            request = await session.post(url=url, json=json, timeout=self.timeout)
...
API_Crystal = CrystalPayAPI(login=settings.CRYSTAL_LOGIN, token=settings.CRYSTAL_API_TOKEN)
